...
db.exec(`
  ...
  -- This is handled below via ALTER TABLE
...
// Migrate existing DB - add password_hash if missing
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    db.prepare(`INSERT INTO users (id, username, display_name, avatar_color, avatar_emoji, password_hash) VALUES (?, ?, ?, ?, ?, ?)`)
      .run(id, username.toLowerCase().trim(), display_name.trim(), avatar_color, avatar_emoji, password_hash);
    const user = db.prepare(`SELECT id, username, display_name, avatar_color, avatar_emoji, created_at FROM users WHERE id = ?`).get(id);
...
app.post('/api/login', async (req, res) => {
  ...
  // Support legacy accounts with no password (let them in, prompt to set one later)
  if (!user.password_hash) {
    const safeUser = { id: user.id, username: user.username, display_name: user.display_name, avatar_color: user.avatar_color, avatar_emoji: user.avatar_emoji };
    return res.json({ success: true, user: safeUser, noPassword: true });
  }
